...
class MetePay {
    ...
    public function pay($order) {
        ...
        if ($result->success) {
            ...
            // 检测是否是移动端浏览器
            $is_mobile = $this->isMobile($_SERVER['HTTP_USER_AGENT']);
            ...
            if($is_mobile){
              ...
              if ($result->data->type === 'qrcode') {
                return [
                    ...
                    'data' => 'alipays://platformapi/startapp?appId=20000067&url='.urlencode($result->data->url)
                ];
    ...
    public function isMobile($user_agent){
      $mobile_agents = "/(Mobile|iPhone|iPod|Android|BlackBerry|Windows Phone)/";
      return preg_match($mobile_agents, $user_agent);
    }
