...
class OrderStatisticsMonth extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'order:statistics_month';
    ...
    {
        $between = [Carbon::now()->subMonth()->startOfDay()->timestamp, Carbon::now()->subMonth()->endOfDay()->timestamp];
        ...
        $data_3 = Order::query()
            ...
            ->get()
            ->toArray();
        $text = '上月新购/续费/升级统计~' . PHP_EOL . PHP_EOL;
        foreach ($keys as $key) {
        ...
        $data_1 = Order::query()
            ...
            ->get()
            ->toArray();
        $text = '上月AFF销量统计~' . PHP_EOL . PHP_EOL;
        foreach ($keys as $key) {
            foreach ($data_1 as $item) {
                if ($item['period'] === $key) {
                    $text .= $this->period[$key] . '数: ' . $item['total_amount_count'] . '笔' . PHP_EOL;
